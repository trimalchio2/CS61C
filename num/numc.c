...
/* 使用PyObject結構體來保存所有對象共同的數據，以及實現GC機制所需要的一些輔助自段*/
/* PyTypeObject就是用來描述一種類型對象的行為的結構體，對於每一種類型對象來說都要有一個 PyTypeObject對象保存與這類型相關的一些數據成員和函數指針*/
/* 並且每個類型對象在初始的時候都會初始化PyTypeObject對象，類型信息就固定下來*/
/* PyTypeObject本身也是一個對象，PyObject_VAR_HEAD定義*/
/* 透過PyTypeObject在C語言層面實現了多態*/
/* print PyObject* object {object->ob_type->tp_print(object)}*/
...
static int init_rand(PyObject *self, int rows, int cols, unsigned int seed, double low, double high) {
    ...
    ((Matrix61c *)self)->shape = PyTuple_Pack(2, PyLong_FromLong(rows), PyLong_FromLong(cols)); /*pytuple to make tuple type attribute*/
    return 0;
}
...
static int Matrix61c_init(PyObject *self, PyObject *args, PyObject *kwds) {
    ...
    if (kwds != NULL) {
        /* return the object dictionary p which has a string key*/
    ...
    if (PyArg_UnpackTuple(args, "args", 1, 3, &arg1, &arg2, &arg3)) {
        ...
        if (arg1 && arg2 && arg3 && PyLong_Check(arg1) && PyLong_Check(arg2) && (PyLong_Check(arg3) || PyFloat_Check(arg3))) {
            /*All entrues in the matrix will be initialized to val*/
...
static PyObject *Matrix61c_add(Matrix61c* self, PyObject* args) {
    ...
    int cols = self->mat->cols;
    int alloc_failed = allocate_matrix(&new_mat, rows, cols);
    if (PyObject_TypeCheck(args, Matrix61c)){
        add_matrix(new_mat, self->mat, args->mat);
    }
}
...
/*Definr a struct PyTypeObject named Matrix61cType to specify the intended behaviors of Python object Matrix61c*/
static PyTypeObject Matrix61cType = {
